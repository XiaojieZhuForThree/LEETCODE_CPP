...
public:
    ...
    int dfs(vector<vector<int>>& grid, int r, int c, int step, bool turned,
        ...
        if (nr >= 0 && nr < n && nc >= 0 && nc < m && grid[nr][nc] == exp) {
            ans = std::max(ans, 1 + dfs(grid, nr, nc, step + 1, turned, d));
        }
        if (!turned) {
            std::pair<int, int> nDir = {dir[d][1], -dir[d][0]};
            int nd = -1;
            for (int i = 0; i < 4; i++) {
            ...
            if (nd != -1 && nr2 >= 0 && nr2 < n && nc2 >= 0 && nc2 < m &&
                grid[nr2][nc2] == exp) {
                ans = std::max(ans, 1 + dfs(grid, nr2, nc2, step + 1, true, nd));
            }
        }
        return ans;
    }
    int lenOfVDiagonal(vector<vector<int>>& grid) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (grid[i][j] == 1) {
                    for (int d = 0; d < 4; d++) {
                        maxi = std::max(maxi, dfs(grid, i, j, 0, false, d));
                    }
                }
            }
        }
