        ...
        for (int parentBought = 0; parentBought <= 1; ++parentBought) {
            ...
            for (const auto& child : childDPs) {
                ...
                for (int b1 = 0; b1 <= budget; ++b1) {
                    ...
                    for (int b2 = 0; b1 + b2 <= budget; ++b2) next[b1 + b2] = max(next[b1 + b2], base[b1] + child.first[b2]);
                }
                base = move(next);
            }
            ...
            if (price <= budget) {
                ...
                for (const auto& child : childDPs) {
                    ...
                    for (int b1 = 0; b1 <= budget; ++b1) {
                        ...
                        for (int b2 = 0; b1 + b2 <= budget; ++b2) next[b1 + b2] = max(next[b1 + b2], baseBuy[b1] + child.second[b2]);
                    }
                    baseBuy = move(next);
                }
